...
local textureList = {
	["empty"] = [[Interface\AddOns\TurboPlates\\Libs\\LibCustomGlow-1.0\AM_29]],
	["white"] = [[Interface\BUTTONS\WHITE8X8]],
	["shine"] = [[Interface\AddOns\TurboPlates\\Libs\\LibCustomGlow-1.0\Artifacts]]
}
...
local function CreateWotLKObjectPool(createObject, resetter)
	local pool = {
		active = {},
		inactive = {},
		resetter = resetter,
	...
	function pool:Acquire()
		local object = tremove(self.inactive)
		local isNew = false
		if not object then
			object = createObject()
			isNew = true
	...
	function pool:Release(object)
		if not object or not self.active[object] then return end
		self.active[object] = nil
		if self.resetter then
			self.resetter(self, object)
		end
		tinsert(self.inactive, object)
	end

	function pool:ReleaseAll()
		local released = {}
		for object in pairs(self.active) do
			tinsert(released, object)
		end
		for _, object in ipairs(released) do
			self:Release(object)
		end
	end
...
local CreateTexturePool = CreateTexturePool or function(parent, layer, subLayer, resetter)
	return CreateWotLKObjectPool(function()
		return parent:CreateTexture(nil, layer, nil, subLayer)
	end, resetter)
end

local CreateFramePool = CreateFramePool or function(frameType, parent, template, resetter)
	template = template ~= "" and template or nil
	return CreateWotLKObjectPool(function()
...
local function configureButtonGlow(f, alpha)
	...
	f.spark:SetAlpha(0)
	f.spark:SetTexture([[Interface\AddOns\TurboPlates\\Libs\\LibCustomGlow-1.0\IconAlert]])
	...
	f.innerGlow:SetTexture([[Interface\AddOns\TurboPlates\\Libs\\LibCustomGlow-1.0\IconAlert]])
	...
	f.innerGlowOver:SetTexture([[Interface\AddOns\TurboPlates\\Libs\\LibCustomGlow-1.0\IconAlert]])
	...
	f.outerGlow:SetTexture([[Interface\AddOns\TurboPlates\\Libs\\LibCustomGlow-1.0\IconAlert]])
	...
	f.outerGlowOver:SetTexture([[Interface\AddOns\TurboPlates\\Libs\\LibCustomGlow-1.0\IconAlert]])
	...
	f.ants:SetAlpha(0)
	f.ants:SetTexture([[Interface\AddOns\TurboPlates\\Libs\\LibCustomGlow-1.0\IconAlertAnts]])
